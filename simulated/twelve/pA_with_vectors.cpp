...
vector<int> pos_verts;
vector<vector<int>> pos_edges;
vector<vector<vector<int>>> len;
...
static inline void connect(string & l1, string & l2, string & w)
{
	...
	if (i == pos_verts[v1])
	{
		pos_verts[v1]++;
		adj[v1].push_back(v1);
		if (pos_edges[v1].size() <= v2)
		{
			pos_edges[v1].resize(v2);
			len[v1].resize(v1);
			len[v1][v2].resize(27);
		}
	}
	...
	if (j == pos_verts[v2])
	{
		pos_verts[v2]++;
		adj[v2].push_back(v1);
		if (pos_edges[v2].size() <= v1)
		{
			pos_edges[v2].resize(v1);
			len[v2].resize(v1);
			len[v2][v1].resize(27);
		}
	}
	...
	if (k == pos_edges[v1][v2])
	{
		...
		len[v1][v2][w0] = wl;
		len[v2][v1][w0] = wl;
		edges[v1][v2].push_back(w0);
		edges[v2][v1].push_back(w0);
	}
	else
	{
		if (wl < len[v1][v2][k])
